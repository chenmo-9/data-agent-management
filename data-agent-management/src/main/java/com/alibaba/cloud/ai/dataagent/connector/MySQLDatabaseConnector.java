...
public class MySQLDatabaseConnector implements DatabaseConnector {
	...
	public DatasourceTestVO testConnection(Datasource datasource) {
		try (var ignored = DriverManager.getConnection(datasource.getUrl(), datasource.getUsername(),
			...
			return new DatasourceTestVO(true, "MySQL connection test succeeded", datasource.getDbType(),
					datasource.getUrl());
		}
		catch (Exception exception) {
			return new DatasourceTestVO(false, "MySQL connection test failed: " + exception.getMessage(),
